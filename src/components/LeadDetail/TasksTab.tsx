...
export default function TasksTab({ lead }: { lead: Lead }) {
  ...
  const pct = lead.tasks.length ? Math.round((done / lead.tasks.length) * 100) : 0;

  const handleAdd = () => {
    if (!newTask.trim()) return;
    addTask(lead.id, newTask.trim());
    setNewTask('');
  };
      ...
      {lead.tasks.length > 0 && (
        <div>
          <div className="flex items-center justify-between text-sm mb-1.5">
            <span className="font-semibold text-gray-700">{done} of {lead.tasks.length} tasks completed</span>
