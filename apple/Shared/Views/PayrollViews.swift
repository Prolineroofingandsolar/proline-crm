    ...
    struct MacPayrollView: View {
        ...
        var body: some View {
            weeklyRunDashboard
                ...
                ) { AddTimesheetView() }.sheet(item: $editingEntry) { AddTimesheetView(entry: $0) }.sheet(item: $editingWorker) {
                    WorkerPaymentDetailsSheet(user: $0)
                }
                ...
                .confirmationDialog(
                    "Confirm payment", isPresented: Binding(get: { pendingPaidRunID != nil }, set: { if !$0 { pendingPaidRunID = nil } }),
                    titleVisibility: .visible
                ) {
                    Button("Mark as paid") {
                        guard let id = pendingPaidRunID, let summary = runSummaries.first(where: { $0.id == id }) else {
                            pendingPaidRunID = nil; return
                        }
                    ...
                    Text("This records the weekly run as paid today. Check the amount and bank transfer before continuing.")
                }
        }
        ...
        private var weeklyRunDashboard: some View {
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top) {
                    ...
                    Spacer()
                    Button {
                        tab = tab == "Timesheets" ? "Runs" : "Timesheets"
                    } label: {
                        Label(
                            tab == "Timesheets" ? "Pay runs" : "Edit timesheets", systemImage: tab == "Timesheets" ? "banknote" : "calendar"
                        )
                    }.buttonStyle(.bordered)
                    Button {
                        quickEntry = QuickTimesheet(userID: "", date: defaultEntryDate)
                    } label: {
                        Label("Add work day", systemImage: "plus")
                    }
        ...
        private var weekDetailStrip: some View {
            HStack(spacing: 10) {
                weekDetail(
                    "Period",
                    "\(weekStart.formatted(.dateTime.day().month(.abbreviated))) – \(weekEnd.formatted(.dateTime.day().month(.abbreviated)))",
                    "calendar", .blue)
                weekDetail("Work recorded", "\(PayrollMath.days(weekEntries).formatted()) days", "clock.fill", Color.accentColor)
                weekDetail("Jobs covered", "\(Set(weekEntries.map(\.leadID)).count)", "briefcase.fill", .purple)
                weekDetail(
                    "Issues", "\(selectedRunSummaries.filter { missingBankDetails($0.user) }.count)", "exclamationmark.triangle.fill",
                    selectedRunSummaries.contains { missingBankDetails($0.user) } ? .red : .green)
            }
        }
        private func weekDetail(_ title: String, _ value: String, _ icon: String, _ colour: Color) -> some View {
            HStack(spacing: 10) {
                Image(systemName: icon).foregroundStyle(colour).frame(width: 32, height: 32).background(
                    colour.opacity(0.1), in: RoundedRectangle(cornerRadius: 8))
                VStack(alignment: .leading, spacing: 2) {
                    Text(title).font(.caption).foregroundStyle(.secondary); Text(value).fontWeight(.semibold)
                }
                Spacer()
            }.padding(11).frame(maxWidth: .infinity).background(.background, in: RoundedRectangle(cornerRadius: 8)).overlay(
        ...
        private var runProgressBanner: some View {
            HStack(spacing: 26) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(
                        "\(weekStart.formatted(.dateTime.day().month(.abbreviated))) – \(weekEnd.formatted(date: .abbreviated, time: .omitted))"
                    ).fontWeight(.semibold)
                    HStack(alignment: .firstTextBaseline, spacing: 8) {
                        Text(selectedNet, format: .currency(code: "GBP")).font(.title.bold())
            ...
            }.padding(20).background(.background, in: RoundedRectangle(cornerRadius: 12)).overlay(
        ...
        private func runStepView(_ number: Int, _ title: String, _ subtitle: String) -> some View {
            VStack(spacing: 5) {
                Text("\(number)").fontWeight(.bold).foregroundStyle(number <= runStep ? .white : .secondary).frame(width: 31, height: 31)
                    .background(number <= runStep ? Color.accentColor : Color.clear, in: Circle()).overlay(Circle().stroke(.quaternary))
                            ...
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 7) {
                                    ...
                                    RoundedRectangle(cornerRadius: 8).stroke(
                                        PayrollMath.key(date) == PayrollMath.key(weekStart)
                                            ? Color.accentColor : Color.secondary.opacity(0.2),
                                        lineWidth: PayrollMath.key(date) == PayrollMath.key(weekStart) ? 2 : 1))
                            }.buttonStyle(.plain)
                        }
                    }
                }
            }.padding(14).frame(width: 310, height: 500, alignment: .top).background(.background, in: RoundedRectangle(cornerRadius: 12))
        ...
        private var workerRunPanel: some View {
            VStack(spacing: 0) {
                ...
                if selectedRunSummaries.isEmpty {
                    ContentUnavailableView(
                        "No time recorded", systemImage: "calendar.badge.plus",
                        description: Text("Add a worker's first work day for this week.")
                    )
                ...
                HStack(spacing: 30) {
                    ...
                    Spacer()
                    if runStep == 1 {
                        Button("Approve submitted run") { updateSelectedRun(to: .scheduled) }.buttonStyle(.borderedProminent).disabled(
                            selectedRunSummaries.isEmpty
                                || selectedRunSummaries.contains(where: { missingBankDetails($0.user) || $0.status == .due }))
                    } else if runStep == 2 {
            ...
            }.frame(maxWidth: .infinity, minHeight: 500, maxHeight: 500).background(.background, in: RoundedRectangle(cornerRadius: 12))
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(.quaternary))
        }
                ...
                HStack(spacing: 12) {
                    Button {
                        withAnimation(.easeInOut(duration: 0.18)) {
                            expandedWorkerID = expandedWorkerID == summary.user.id ? nil : summary.user.id
                        }
                    } label: {
                        HStack(spacing: 12) {
                            Circle().fill(Color.accentColor.opacity(0.13)).frame(width: 42, height: 42).overlay(
                                Text(initials(summary.user.name)).font(.caption.bold()).foregroundStyle(Color.accentColor))
                            VStack(alignment: .leading, spacing: 3) {
                                Text(summary.user.name).fontWeight(.semibold)
                                Text(
                                    "\(summary.days.formatted()) days · \(workerEntries(summary.user).count) entries · CIS \(summary.rate)%"
                                ).font(.caption).foregroundStyle(.secondary)
                                Label(
                                    summary.status == .due ? "Not submitted" : summary.status.displayName,
                                    systemImage: summary.status == .due ? "clock" : "checkmark.circle.fill"
                                )
                                .font(.caption2.bold()).foregroundStyle(summary.status == .due ? Color.secondary : Color.green)
                            }
                            Image(systemName: expandedWorkerID == summary.user.id ? "chevron.up" : "chevron.down").font(.caption)
                    ...
                    if missingBankDetails(summary.user) {
                        ...
                        Label(summary.status.displayName, systemImage: summary.status == .due ? "clock" : "checkmark.circle.fill").font(
                            .caption.bold()
                        ).foregroundStyle(
                            summary.status == .paid
                                ? .green
                                : summary.status == .scheduled ? .blue : summary.status == .submitted ? Color.accentColor : .secondary
                        ).frame(width: 105, alignment: .leading)
                    }
                    Menu {
                        Button("Edit payment details") { editingWorker = summary.user };
                        Button("Log work day") { quickEntry = QuickTimesheet(userID: summary.user.id, date: defaultEntryDate) }
                    } label: {
        ...
        private func workerEntries(_ user: CRMUser) -> [TimesheetEntry] {
            weekEntries.filter { $0.userID == user.id }.sorted { $0.date < $1.date }
        }
        private func adminEntries(_ user: CRMUser) -> [TimesheetEntry] {
            adminWeekEntries.filter { $0.userID == user.id }.sorted { $0.date < $1.date }
        }
        ...
        private func copyStatus(_ user: CRMUser) -> String {
            adminEntries(user).isEmpty
                ? "No office copy yet" : timesheetCopiesMatch(user) ? "Matches office copy" : "Does not match office copy"
        }
        private func workerDayDetails(_ summary: RunSummary) -> some View {
            VStack(spacing: 0) {
                ...
                ForEach(workerEntries(summary.user)) { entry in
                    HStack {
                        Text((SupabaseService.date(from: entry.date) ?? .now).formatted(.dateTime.weekday(.abbreviated).day())).frame(
                        ...
                        Text(entry.type == "half" ? "Half day" : entry.type == "off" ? "Off" : "Full day").frame(
                            width: 85, alignment: .leading)
                    ...
                    }.font(.caption).padding(.horizontal, 24).frame(height: 49).background(Color.secondary.opacity(0.035)).overlay(
                        alignment: .bottom
                    ) { Divider().padding(.leading, 24) }
                }
                Button {
                    quickEntry = QuickTimesheet(userID: summary.user.id, date: defaultEntryDate)
                } label: {
                    Label("Add to office copy", systemImage: "plus")
                }.buttonStyle(.plain).foregroundStyle(Color.accentColor).padding(.vertical, 10)
            }.background(Color.secondary.opacity(0.025))
        }
        private func runTotal(_ title: String, _ value: Double) -> some View {
            VStack(alignment: .leading, spacing: 2) {
        ...
        private func missingBankDetails(_ user: CRMUser) -> Bool {
            (user.bankAccountNumber ?? "").isEmpty || (user.bankSortCode ?? "").isEmpty
        }
        private func weekStatus(_ summaries: [RunSummary]) -> String {
            summaries.isEmpty
                ? "Empty"
                : summaries.allSatisfy { $0.status == .paid }
                    ? "Paid"
                    : summaries.allSatisfy { $0.status == .scheduled || $0.status == .paid }
                        ? "Approved" : summaries.contains { $0.status == .submitted } ? "Submitted" : "In progress"
        }
        private func weekStatusColor(_ summaries: [RunSummary]) -> Color {
            summaries.allSatisfy { $0.status == .paid } && !summaries.isEmpty
                ? .green : summaries.allSatisfy { $0.status != .due } && !summaries.isEmpty ? .blue : Color.accentColor
        }
        private func updateSelectedRun(to status: PaymentStatus) {
            Task {
                for summary in selectedRunSummaries {
                    await appState.setPaymentStatus(userID: summary.user.id, weekStart: summary.weekStart, status: status)
                }
            }
        }
                ...
                VStack(alignment: .leading, spacing: 20) {
                    HStack(spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(
                                "\(weekStart.formatted(.dateTime.day().month(.wide))) – \(weekEnd.formatted(.dateTime.day().month(.wide).year()))"
                            ).font(.title2.bold())
                    ...
                    HStack(spacing: 14) {
                        ...
                        timesheetSummaryMetric(
                            "BEFORE TAX", PayrollMath.gross(weekEntries).formatted(.currency(code: "GBP").precision(.fractionLength(0))),
                            Color.accentColor)
                        timesheetSummaryMetric("AFTER TAX", weekNet.formatted(.currency(code: "GBP").precision(.fractionLength(0))), .green)
                        Spacer(minLength: 0)
                    }
                }.padding(20).background(Color.accentColor.opacity(0.045), in: RoundedRectangle(cornerRadius: 12)).overlay(
                ...
                VStack(spacing: 0) {
                    HStack(spacing: 0) {
                        ...
                        ForEach(0..<5, id: \.self) { offset in
                            ...
                            VStack(spacing: 4) {
                                ...
                                Text(date.formatted(.dateTime.day())).font(.headline).foregroundStyle(
                                    Calendar.current.isDateInToday(date) ? Color.accentColor : Color.primary)
                            }.frame(maxWidth: .infinity)
                        }
                        Text("BEFORE TAX").frame(width: 100, alignment: .trailing)
                        Text("AFTER TAX").frame(width: 100, alignment: .trailing)
                    }
                    .font(.caption2.bold()).foregroundStyle(.secondary).padding(.horizontal, 18).frame(height: 58).background(
                    ...
                    ScrollView {
                        LazyVStack(spacing: 0) {
                            ...
                            if workers.isEmpty {
                                ContentUnavailableView(
                                    "No team members", systemImage: "person.2",
                                    description: Text("Create worker accounts in Settings first.")
                                )
                                .frame(minHeight: 220)
                            }
                        }
                    ...
                    }.font(.caption).foregroundStyle(.secondary).padding(.horizontal, 18).frame(height: 48).background(
                        Color.secondary.opacity(0.025)
                    ).overlay(alignment: .top) { Divider() }
                }.background(.background, in: RoundedRectangle(cornerRadius: 12)).overlay(
                    RoundedRectangle(cornerRadius: 12).stroke(.quaternary)
                ).clipShape(RoundedRectangle(cornerRadius: 12)).shadow(color: .black.opacity(0.035), radius: 10, y: 3)
            }.frame(maxHeight: .infinity, alignment: .top)
        }
        private func timesheetSummaryMetric(_ title: String, _ value: String, _ colour: Color) -> some View {
            VStack(alignment: .leading, spacing: 7) {
                Text(title).font(.caption.bold()).foregroundStyle(.secondary)
                Text(value).font(.title2.weight(.semibold)).foregroundStyle(colour)
            }.padding(14).frame(width: 210, alignment: .leading).frame(minHeight: 78).background(
        ...
        private func workerRow(_ user: CRMUser) -> some View {
            ...
            return HStack(spacing: 0) {
                Button {
                    ...
                    HStack {
                        Circle().fill(Color.accentColor.opacity(0.12)).frame(width: 34, height: 34).overlay(
                            Text(user.name.prefix(1)).font(.caption.bold()).foregroundStyle(Color.accentColor))
                    ...
                    if let entry = entries.first(where: { $0.date == date }) {
                        ...
                        Button {
                            ...
                            VStack(spacing: 2) {
                                ...
                                if entry.type != "off", let reference = job?.jobRef { Text(reference).font(.caption2).lineLimit(1) }
                            }.foregroundStyle(entry.type == "off" ? Color.secondary : Color.accentColor).frame(
                                maxWidth: .infinity, minHeight: 54
                            ).background(
                                entry.type == "off" ? Color.secondary.opacity(0.07) : Color.accentColor.opacity(0.09),
                                in: RoundedRectangle(cornerRadius: 8)
                            ).overlay(
                                RoundedRectangle(cornerRadius: 8).stroke(
                                    entry.type == "off" ? Color.secondary.opacity(0.12) : Color.accentColor.opacity(0.16))
                        ...
                        Button {
                            quickEntry = QuickTimesheet(
                                userID: user.id, date: Calendar.current.date(byAdding: .day, value: offset, to: weekStart)!)
                        } label: {
                            VStack(spacing: 3) {
                                Image(systemName: "plus"); Text("Add day").font(.caption2)
                            }.foregroundStyle(.tertiary).frame(maxWidth: .infinity, minHeight: 54).background(
                                Color.secondary.opacity(0.025), in: RoundedRectangle(cornerRadius: 8)
                            ).overlay(RoundedRectangle(cornerRadius: 8).stroke(Color.secondary.opacity(0.10))).padding(.horizontal, 4)
            ...
            ScrollView {
                LazyVStack(spacing: 12) {
                    ForEach(workerBalances, id: \.user.id) { summary in
                        VStack(spacing: 12) {
                            HStack {
                                Circle().fill(Color.accentColor.opacity(0.12)).frame(width: 44, height: 44).overlay(
                                    Text(summary.user.name.prefix(1)).font(.headline).foregroundStyle(Color.accentColor))
                                VStack(alignment: .leading) {
                                ...
                                VStack(alignment: .trailing) {
                                    ...
                                    ).font(.title3.bold()).foregroundStyle(summary.balance <= 0.01 ? .green : Color.accentColor)
        ...
        private var runs: some View {
            ScrollView {
                VStack(spacing: 16) {
                    HStack(spacing: 12) {
                        PayrollStatusMetric(
                            "Due", runSummaries.filter { $0.status == .due }.reduce(0) { $0 + $1.net }, Color.accentColor)
                        PayrollStatusMetric(
                            "Scheduled",
                                ...
                                Menu(summary.status.displayName) {
                                    ForEach(PaymentStatus.allCases) { state in
                                        Button(state.displayName) {
                                            if state == .paid {
                                                pendingPaidRunID = summary.id
                                            } else {
                                                Task {
                                                    await appState.setPaymentStatus(
                                                        userID: summary.user.id, weekStart: summary.weekStart, status: state)
                                                }
                                            }
                                        }
                                    }
    ...
    private struct PayrollStatusMetric: View {
        ...
        var body: some View {
            VStack(alignment: .leading) {
                ...
                tint.opacity(0.07), in: RoundedRectangle(cornerRadius: 8))
        }
    }
...
struct CISView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                HStack {
                    ...
                    }.buttonStyle(.borderedProminent).disabled(rows.isEmpty)
                }
                HStack(spacing: 12) {
                    CISMetric("Gross labour", totalGross, .blue)
                    CISMetric("CIS deducted", totalDeduction, .red)
                    CISMetric("Net payable", totalNet, .green)
                    CISMetric("Contractors", Double(rows.count), Color.accentColor, currency: false)
                }
                VStack(spacing: 0) {
                    HStack {
        ...
        }.navigationTitle("CIS").fileExporter(
            isPresented: $exporting, document: exportDocument, contentType: .commaSeparatedText,
            defaultFilename: String(format: "ProLine-CIS-%04d-%02d", year, month)
        ) { result in if case .failure = result { appState.errorMessage = "The CIS report could not be exported." } }
    }
    private func prepareExport() {
        ...
        lines += rows.map { row in
            [
                period, row.user.name, row.user.utrNumber ?? "", "\(row.user.cisRate ?? 20)", row.days.formatted(),
                String(format: "%.2f", row.gross), String(format: "%.2f", row.deduction), String(format: "%.2f", row.net),
            ].map(csvField).joined(separator: ",")
        }
        lines.append(
            [
                period, "TOTAL", "", "", "", String(format: "%.2f", totalGross), String(format: "%.2f", totalDeduction),
