...
/// Every stage, for moves that are not the next step in the workflow.
...
private struct MobilePipelineOpportunity:View {
    ...
    var body:some View{VStack(spacing:0){NavigationLink{LeadDetailView(leadID:lead.id)}label:{MobilePipelineCard(lead:lead,tint:tint)}.buttonStyle(.plain);HStack(spacing:4){if ContactLinks.telephone(lead.phone) != nil{Button{if let url=appState.beginCall(to:lead){openURL(url)}}label:{Label("Call",systemImage:"phone.fill").frame(maxWidth:.infinity)}}else{Label("No phone",systemImage:"phone.slash").frame(maxWidth:.infinity).foregroundStyle(.secondary)};Divider().frame(height:20);Button{completeNext()}label:{if completing{ProgressView().frame(maxWidth:.infinity)}else{Label(next == nil ? "No task":"Complete",systemImage:next == nil ? "checklist.unchecked":"checkmark.circle").frame(maxWidth:.infinity)}}.disabled(next == nil || completing);Divider().frame(height:20);Menu{ForEach(stages){destination in Button{Task{await appState.move(lead,to:destination)}}label:{Label(destination.displayName,systemImage:stageIconMobile(destination))}};Divider();Button{showingSchedule=true}label:{Label(lead.startDate == nil ? "Schedule job":"Edit schedule",systemImage:"calendar.badge.clock")};Button{confirmingJobCompletion=true}label:{Label("Complete job",systemImage:"archivebox")}}label:{Label("Move",systemImage:"arrow.right.circle").frame(maxWidth:.infinity)}}.font(.caption.bold()).buttonStyle(.plain).foregroundStyle(tint).padding(.horizontal,8).frame(height:42).background(.background).overlay(alignment:.top){Divider()}}
...
struct AddLeadView: View {
    ...
    static let jobTypes = ["Roof Repair", "New Roof", "Flat Roof", "Solar Installation", "Solar + Battery", "Guttering", "Fascias & Soffits", "Chimney Repair", "Roof Inspection", "Other"]
    static let sources = ["Website", "Referral", "Google", "Facebook", "Checkatrade", "MyBuilder", "Cold Call", "Returning Customer", "Other"]
                ...
                VStack(alignment: .leading, spacing: 18) {
                    formSection("Contact", icon: "person") {
                        ...
                        twoColumns {
                            ...
                            pickerField("Lead source", icon: "building.2", selection: $source, options: Self.sources)
                        }
                    ...
                    formSection("Enquiry", icon: "clipboard") {
                        twoColumns {
                            pickerField("Service required", required: true, icon: "wrench.and.screwdriver", selection: $jobType, options: Self.jobTypes)
                        } trailing: {
                            currencyField
                        }
                        depositField
