...
struct DashboardJobsMap: View {
    ...
    private var header: some View {
        ...
        HStack(spacing: 14) {
            ...
            Button { Task { await locateMissing() } } label: {
            ...
            Button { openRoute() } label: { Label("Route", systemImage: "arrow.triangle.turn.up.right.diamond") }.disabled(mapped.isEmpty)
        }
        .padding(14)
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            if locating {
                Label { Text("Locating addresses…") } icon: { ProgressView().controlSize(.mini) }
    ...
    private var map: some View {
        Map(position: $camera, selection: $selection) {
            ForEach(mapped) { lead in
                Marker(lead.name, systemImage: markerIcon(lead), coordinate: CLLocationCoordinate2D(latitude: lead.lat!, longitude: lead.lng!))
                    .tint(markerTint(lead)).tag(lead.id)
            }
        }
        .mapStyle(.standard(elevation: .realistic, emphasis: .muted))
        .mapControls { MapCompass(); MapScaleView(); MapPitchToggle(); MapUserLocationButton() }
    ...
    private var stats: some View {
        ...
        HStack(spacing: 12) {
            ...
            Label("\(mapped.filter { $0.stage == .inProgress }.count) active", systemImage: "hammer.fill")
                .foregroundStyle(.red)
            if failedCount > 0 { Text("\(failedCount) not found").foregroundStyle(.orange) }
        }
        .font(.caption.bold()).padding(9).background(.ultraThickMaterial, in: RoundedRectangle(cornerRadius: 9)).shadow(radius: 4)
        #else
        HStack(spacing: 14) {
            compactStat(mapped.filter { $0.stage == .surveyBooked }.count, "ruler", .orange)
    ...
    private func jobPanel(_ lead: Lead) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading) { Text(lead.name).font(.headline); Text("\(lead.jobRef) · \(lead.stage.displayName)").font(.caption).foregroundStyle(.secondary) }
                Spacer(); Button { selection = nil } label: { Image(systemName: "xmark") }.buttonStyle(.plain)
            }
            Label(lead.address, systemImage: "mappin").font(.subheadline).lineLimit(2)
            HStack { Text(lead.jobType); Spacer(); Text(lead.value, format: .currency(code: "GBP")).fontWeight(.semibold) }.font(.subheadline)
            HStack {
                Button { openDirections(to: lead) } label: { Label("Directions", systemImage: "car") }.buttonStyle(.borderedProminent)
                NavigationLink { LeadDetailView(leadID: lead.id) } label: { Label("Open", systemImage: "arrow.up.right.square") }.buttonStyle(.bordered)
            }
        }
        .padding(15).background(.ultraThickMaterial, in: RoundedRectangle(cornerRadius: 12)).shadow(color: .black.opacity(0.15), radius: 10, y: 4)
    }
    ...
    private func openDirections(to lead: Lead) {
        ...
        let item = MKMapItem(placemark: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: lat, longitude: lng))); item.name = "\(lead.name) · \(lead.jobRef)"
        item.openInMaps(launchOptions: [MKLaunchOptionsDirectionsModeKey: MKLaunchOptionsDirectionsModeDriving])
    }

    private func openRoute() {
        let stops = mapped.prefix(10).map { lead -> MKMapItem in
            let item = MKMapItem(placemark: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: lead.lat!, longitude: lead.lng!))); item.name = lead.name; return item
        }
        MKMapItem.openMaps(with: [MKMapItem.forCurrentLocation()] + stops, launchOptions: [MKLaunchOptionsDirectionsModeKey: MKLaunchOptionsDirectionsModeDriving])
    }

    private func markerIcon(_ lead: Lead) -> String { lead.stage == .surveyBooked ? "ruler" : (lead.stage == .inProgress ? "hammer.fill" : "calendar.badge.clock") }
    private func markerTint(_ lead: Lead) -> Color { switch lead.stage { case .surveyBooked: .orange; case .inProgress: .red; case .scheduled: .blue; default: .gray } }
}
