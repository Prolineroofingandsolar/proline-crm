...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            if appState.isAdmin {
                ...
                } header: { Text("Business details") } footer: { Text("Printed on quotes. Stored on this device.") }

                Section {
                    ForEach(appState.users) { user in
                        LabeledContent(user.name) { Text(user.role.capitalized).foregroundStyle(.secondary) }
                    }
                    Button { showingInviteWorker = true } label: { Label("Invite team member", systemImage: "paperplane") }
                    Button { showingAddUser = true } label: { Label("Add user", systemImage: "person.crop.circle.badge.plus") }
                } header: { Text("Team") } footer: { Text("Invited workers choose their own password and enter their own pay, CIS and bank details.") }
            }
            ...
            .onChange(of: [notifyTeam, notifySurveys, notifyJobStarts, notifyTasks, notifyPayments]) { Task { await appState.scheduleNotifications() } }
            ...
            if appState.isAdmin {
                ...
                } header: { Text("Gmail assistant") } footer: { Text("Important customer emails become tasks and suggested replies are saved as Gmail drafts. Nothing is ever sent automatically.") }
        ...
        .fileExporter(isPresented: $exporting, document: exportDocument, contentType: .commaSeparatedText, defaultFilename: "ProLine-CRM-Export-\(SupabaseService.today)") { result in
    ...
    private func prepareExport() {
        ...
        lines += appState.leads.map { "lead,\(csv($0.id)),\(csv($0.name)),\(csv($0.jobRef)),\(csv($0.stage.rawValue)),\($0.value),\(csv($0.updatedAt))" }
        lines += appState.contacts.map { "contact,\(csv($0.id)),\(csv($0.name)),,,0,\(csv($0.createdAt))" }
        lines += appState.visibleGeneralTasks.map { "task,\(csv($0.id)),\(csv($0.title)),,\($0.completed ? "completed" : "open"),0,\(csv($0.dueDate ?? ""))" }
