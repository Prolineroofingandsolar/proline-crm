...
struct PhoneActionMenu: View {
    ...
    private var compact: String { number.filter { $0.isNumber || $0 == "+" } }
    private var international: String { compact.hasPrefix("0") ? "+44" + compact.dropFirst() : compact }
    var body: some View {
        Menu {
            if let url = URL(string: "tel:\(compact)") { Link(destination: url) { Label("Call", systemImage: "phone.fill") } }
            if let url = URL(string: "sms:\(compact)") { Link(destination: url) { Label("Text message", systemImage: "message.fill") } }
            if let url = URL(string: "https://wa.me/\(international.replacingOccurrences(of: "+", with: ""))") { Link(destination: url) { Label("WhatsApp", systemImage: "bubble.left.and.bubble.right.fill") } }
            Divider()
...
struct LeadDetailView: View {
    ...
    private func macProfileHeader(_ lead: Lead) -> some View {
        HStack(spacing: 20) {
            ...
            HStack(spacing: 12) {
                ...
                if let mailURL = URL(string: "mailto:\(lead.email)"), !lead.email.isEmpty { Link(destination: mailURL) { Label("Email", systemImage: "envelope").frame(width: 112, height: 42).background(.background, in: RoundedRectangle(cornerRadius: 8)).overlay(RoundedRectangle(cornerRadius: 8).stroke(.quaternary)) }.buttonStyle(.plain) }
    ...
    private func macInfo(_ lead: Lead) -> some View {
        VStack(alignment: .leading, spacing: 24) {
            detailSection("Contact") {
                ...
                detailRow("Email", icon: "envelope", value: lead.email, link: lead.email.isEmpty ? nil : URL(string: "mailto:\(lead.email)"))
    ...
    private func complete(_ task: CRMTask, for lead: Lead) { var changed = lead; guard let index = changed.tasks.firstIndex(where: { $0.id == task.id }) else { return }; changed.tasks[index].completed = true; changed.tasks[index].completedDate = SupabaseService.today; let done=changed.tasks.filter{$0.completed}.count;changed.progress=changed.tasks.isEmpty ? 0:Int((Double(done)/Double(changed.tasks.count)*100).rounded()); Task { await appState.saveLead(changed) } }
    private func addNote(to lead: Lead) { let value = newNote.trimmingCharacters(in: .whitespacesAndNewlines); guard !value.isEmpty else { return }; var changed = lead; changed.notes.insert(CRMNote(id: UUID().uuidString, content: value, date: SupabaseService.today, author: appState.currentUser?.name ?? ""), at: 0); changed.updatedAt=SupabaseService.now;newNote = ""; Task { await appState.saveLead(changed) } }
    ...
    private func notes(_ lead: Lead) -> some View { List { Section("Add Note") { HStack { TextField("Note", text:$newNote, axis:.vertical); Button("Add") { var changed=lead; changed.notes.insert(CRMNote(id:UUID().uuidString,content:newNote,date:SupabaseService.today,author:appState.currentUser?.name ?? ""),at:0); newNote=""; Task { await appState.saveLead(changed) } }.disabled(newNote.isEmpty) } }; ForEach(lead.notes) { note in VStack(alignment:.leading) { Text(note.content); Text("\(note.author) · \(note.date)").font(.caption).foregroundStyle(.secondary) } } } }
